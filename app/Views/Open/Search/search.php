...
<div class="container mb-lg-5 pb-5" id="boutique">
    <ul class="nav nav-tabs align-items-end" id="boutiqueTabs">
        <li class="nav-item pb-0">
            ...
            <a href="#recherche-articles" class="nav-link <?= $active ?>" data-bs-toggle="tab"><?= $this->bi('newspaper') ?> Articles (<?= $articles->totalRecords() ?>)</a>
        </li>
        <li class="nav-item pb-0">
            <?php $active = $controller->router()->params('tab') == 'pro' ? 'active' : ''; ?>
            <a href="#recherche-professionnels" class="nav-link <?= $active ?>" data-bs-toggle="tab"><?= $this->bi('person') ?> Professionnels (<?= $professionals->totalRecords() ?>)</a>
        </li>
        <li class="nav-item pb-0">
            <?php $active = $controller->router()->params('tab') == 'mov' ? 'active' : ''; ?>

            <a href="#recherche-films" class="nav-link <?= $active ?>" data-bs-toggle="tab"><?= $this->bi('film') ?> Films (<?= $movies->totalRecords() ?>)</a>
        </li>
        <li class="nav-item pb-0">
            <?php $active = $controller->router()->params('tab') == 'org' ? 'active' : ''; ?>

            <a href="#recherche-organisations" class="nav-link <?= $active ?>" data-bs-toggle="tab"><?= $this->bi('building') ?> Organisations (<?= $organisations->totalRecords() ?>)</a>
        </li>
    </ul>
